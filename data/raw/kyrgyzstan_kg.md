...
Source: https://www.twilio.com/en-us/guidelines/kg/sms
...
| ISO code | The International Organization for Standardization two character representation for the given locale. | KG |
| Region | --- | Europe |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 437 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +996 |
...
| Provisioning time | Provisioning is the process of getting the sender ID approved and registered with mobile networks (depending on country requirements). Provisioning time is how long this process takes in the given country. | --- | N/A | --- |
| UCS-2 support | --- | --- | Supported | --- |
| Use case restrictions | --- | --- | The network Megacom does not support Numeric Sender IDs. Use Alphanumeric Sender IDs to ensure message delivery. Messages submitted with a Numeric Sender ID will be delivered on a best effort basis and the Numeric Sender ID will be replaced with a generic Alphanumeric one outside Twilio's platform | --- |
| Best practices | --- | --- | --- | --- |
...
| Service restrictions | No sender registration is required for international senders. |
| Service provisioning | No sender registration is required for international senders. |
...
| Country regulations | Opt ins are mandatory. Messages must comply with the norms of the current legislation of the Kyrgyz Republic. Night time restrictions - SMS which haven't been requested by subscribers from 11:00 PM to 8:00 AM or 10:00 PM to 9:00 AM (this depends on MNO) are forbidden. |
| Country restrictions | Night time restrictions. SPAM and fraud is forbidden. Messages degrading human dignity, propagandizing violence, racial and inter-ethnic enmity, violating public security, undermining the foundations of the constitutional order, state security and the security of mankind are forbidden. |
| Country recommendations | Traffic is divided into local and international traffic. Registration for local senders is mandatory. Get opt ins in the form of a consent from each user before you send any traffic towards them. |