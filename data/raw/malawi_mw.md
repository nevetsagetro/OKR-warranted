...
Source: https://www.twilio.com/en-us/guidelines/mw/sms
...
| --- | --- | --- |
| Locale name | --- | Malawi |
| ISO code | The International Organization for Standardization two character representation for the given locale. | MW |
| Region | --- | Middle East & Africa |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 650 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +265 |
...
| Best practices | --- | --- | Dynamic Numeric Sender IDs are not supported by the network Airtel. | --- |
...
- Malawi Phone Number: Yes
- Malawi Short Code: Yes
...
| Service restrictions | There's traffic differentiation based on origin. Sender registration is required. Businesses need to provide proper documentation for sending local-origin traffic. Before you start sending any content towards Malawi for the first time, contact your dedicated account manager or [Support](mailto:support@infobip.com). |
| Service provisioning | Service provisioning for local-origin traffic is up to 6 days. Since no prior registration of senders sending international-origin traffic is necessary, service provisioning should be available within a day. |
| Sender availability | Alpha sender allowed. Numeric and generic senders will be blocked. |
| Sender provisioning | Up to 6 days for traffic of local origin. For traffic of international origin, sender provisioning should be available within a day |
| Two-way | Two-way services are available for local traffic only. Short Code setup available with Standard, Premium and Free to End Users billing models. |
| Two-way provisioning | Estimated provisioning time is 3 months. |
| Country regulations | No specific regulations for A2P messaging. |
| Country restrictions | Alcohol, tobacco, religious traffic is not allowed. Gambling traffic is allowed. |
