...
Source: https://www.twilio.com/en-us/guidelines/sk/sms
...
| ISO code | The International Organization for Standardization two character representation for the given locale. | SK |
| Region | --- | Europe |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 231 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +421 |
...
| UCS-2 support | --- | --- | --- | --- |
| Use case restrictions | --- | --- | --- | --- |
| Best practices | --- | --- | Network Operators in Slovakia do not support International Longcode Sender ID. Messages submitted with an International Longcode will be delivered but the Sender ID will be replaced via a generic Alpha one outside Twilio's platform. Twilio suggests using an Alpha Sender ID in Slovakia. | --- |
...
- Slovakia Phone Number: No
- Slovakia Short Code: No
...
| Service provisioning | 1 day  to configure the default account setup. More if it is a specific setup (it could depend on the supplier). |
| Sender availability | - Alpha - Short Code - Virtual Long Numbers - Dynamic Generic sender is allowed. |
...
| Two-way provisioning | The setup can take up to 2 weeks. |
| Country regulations | Messages must contain opt out options. |
| Country restrictions | Gambling traffic is allowed only if end users are subscribed. |
