...
Source: https://www.twilio.com/en-us/guidelines/gr/sms
...
| --- | --- | --- |
| Locale name | --- | Greece |
| ISO code | The International Organization for Standardization two character representation for the given locale. | GR |
| Region | --- | Europe |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 202 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +30 |
...
| Use case restrictions | --- | N/A | Twilio recomends to avoid using generic sender IDs |
| Best practices | --- | N/A | Twilio suggests avoiding the use of any generic Alphanumeric Sender ID as they tend to get blocked from local operators |
...
- Greece Phone Number: No
- Greece Short Code: No
...
| Sender availability | Dynamic alphanumeric and Short Code |
...
| Two-way provisioning | The setup can take up to 4 weeks, depending on the provider. |
| Country regulations | No specific country regulations. |
| Country restrictions | Max. length for numeric senders is 11 digits (numbers longer than that will be cut). Allowed characters in the Sender ID field are: a-z, A-Z, 0-9 . (Full Stop),Space, - (dash). All other characters will be replaced (NULL). |
