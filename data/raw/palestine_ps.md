...
Source: https://www.twilio.com/en-us/guidelines/ps/sms
...
| ISO code | The International Organization for Standardization two character representation for the given locale. | PS |
| Region | --- | Middle East & Africa |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 425 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +970 |
...
| Use case restrictions | --- | --- | Content related to personal loans, gambling, pornography, or cryptocurrency is prohibited. |
| Best practices | --- | --- | --- |
...
| Best practices | --- | N/A | Numeric Sender ID is not supported in Palestine. Submission by International Long Codes will be attempted on a best-effort basis. The Long Code will be overwritten with a generic Alphanumeric Sender ID outside the Twilio platform. | --- |
...
| Concatenated message | Standard 160 characters. Concatenated messages supported. |
| Service restrictions | Sender registration is a required for both networks (Jawwal and Wataniya) on local level. If the sender is a local brand or business, authorization letter may be requested. Traffic is separated only on local and international. |
| Service provisioning | The setup depends on sender provisioning time (depending on the network). |
| Sender availability | Alpha and numeric senders supported. |
| Sender provisioning | The average sender registration process time depends solely on network providers. Usually takes about 2 days. |
| Two-way | No two-way SMS options currently available. |
| Two-way provisioning | / |
| Country regulations | Generally, A2P SMS traffic is divided into local and international. No traffic separation into promo and transactional. |
| Country restrictions | Loan-related, pornography, and cryptocurrency is forbidden. |
| Country recommendations | Allowed type of traffic for Jawwal network: - Palestinian Businesses and Brands. - International brands with branches in Palestine. - Will be available for Jawwal Israel and Palestinian Territory. Restricted for Jawwal and Wataniya Mobile - International A2P senderIDs will be rejected by Jawwal Allowed type of traffic for Watanya Mobile (Ooredoo) network: - NEW - 23.12. - Only local traffic now - International A2P senderIDs will be rejected by Ooredoo. - Unrestricted (A2P, OTT, Promotions, ... ) - Will be available for Watanya Israel and Palestinian Territory. |