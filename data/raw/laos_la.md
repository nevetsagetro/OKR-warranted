...
Source: https://www.twilio.com/en-us/guidelines/la/sms
...
| --- | --- | --- |
| Locale name | --- | Laos |
| ISO code | The International Organization for Standardization two character representation for the given locale. | LA |
| Region | --- | Asia |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 457 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +856 |
...
| Use case restrictions | --- | --- | On Laos networks, only basic characters A–Z , a-z, and 0–9 are guaranteed to be preserved. Other characters may be removed or replaced with alternatives. Please use UCS2 encoding (for example, including the Lao script) if you intend to send SMS containing characters outside this range. |
| Best practices | --- | --- | --- |
...
| UCS-2 support | --- | --- | Supported | --- |
| Use case restrictions | --- | --- | On Laos networks, only basic characters A–Z , a-z, and 0–9 are guaranteed to be preserved. Other characters may be removed or replaced with alternatives. Please use UCS2 encoding (for example, including the Lao script) if you intend to send SMS containing characters outside this range. | --- |
| Best practices | --- | --- | SMS submission to Unitel network in Laos via global SMS-capable number will be overwritten with a generic Alphanumeric Sender ID. We recommend to use alphanumeric sender ID when sending SMS to Unitel network in Laos. | --- |
