...
Source: https://www.twilio.com/en-us/guidelines/lb/sms
...
| --- | --- | --- |
| Locale name | --- | Lebanon |
| ISO code | The International Organization for Standardization two character representation for the given locale. | LB |
| Region | --- | Middle East & Africa |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 415 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +961 |
...
| UCS-2 support | --- | N/A | Yes | N/A |
| Use case restrictions | --- | N/A | Numeric sender ID to Lebanon would be overwritten with generic alphanumeric sender ID outside the Twilio platform and delivery would be attempted on best effort basis only. Twilio highly recommends sending messages with alphanumeric sender ID. | N/A |
...
- Lebanon Phone Number: No
- Lebanon Short Code: No
...
| Service restrictions | No specific restrictions. Local SMS termination is allowed only for local brands (registration is not mandatory, but can be asked for). |
| Service provisioning | Immediately for international SMS termination. Can last up to 2 days for local SMS termination. |
| Sender availability | Alpha senders allowed. |
| Sender provisioning | Immediately, since GWs for all networks are full dynamic. For local senders, proof of local entity can be asked prior using service. |
| Two-way | No two-way SMS options currently available. |
| Two-way provisioning | / |
| Country regulations | Traffic is divided into local and international. The company must have a local presence in Lebanon to be able to use local SMS connections. |
| Country restrictions | No specific restrictions, but sometimes MNOs can request for the opt-out link to be added within promo messages. |
| Country recommendations | Gambling, betting as well as SPAM, loan traffic, Crypto, Forex and adult, political or religious content is likely to be blocked by the Lebanese operators. |