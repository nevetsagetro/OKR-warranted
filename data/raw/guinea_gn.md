...
Source: https://www.twilio.com/en-us/guidelines/gn/sms
...
| --- | --- | --- |
| Locale name | --- | Guinea |
| ISO code | The International Organization for Standardization two character representation for the given locale. | GN |
| Region | --- | Middle East & Africa |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 611 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +224 |
...
| Use case restrictions | --- | Promotional content is not allowed | Dynamic Alphanumeric Sender ID is not allowed in MTN Network |
| Best practices | --- | Please refrain from requesting generic sender IDs like InfoSMS, INFO, Verify, Notify etc to avoid being blocked by network operators. | Twilio suggests using a pre-registered Alphanumeric Sender ID in Guinea |
...
| UCS-2 support | --- | --- | --- | --- |
| Use case restrictions | --- | --- | Numeric International sender ID is not supported to the MTN network in Guinea. Messages submitted with numeric would result in delivery failure. Send only with alphanumeric sender ID to this network. | --- |
| Best practices | --- | --- | Twilio suggests using a pre-registered Alphanumeric Sender ID in Guinea | --- |
...
- Guinea Phone Number: Yes
- Guinea Short Code: Yes
