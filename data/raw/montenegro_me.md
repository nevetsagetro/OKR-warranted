...
Source: https://www.twilio.com/en-us/guidelines/me/sms
...
| ISO code | The International Organization for Standardization two character representation for the given locale. | ME |
| Region | --- | Europe |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 297 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +382 |
...
| Provisioning time | Provisioning is the process of getting the sender ID approved and registered with mobile networks (depending on country requirements). Provisioning time is how long this process takes in the given country. | --- | --- | --- |
| UCS-2 support | --- | --- | Yes | --- |
| Use case restrictions | --- | --- | Dynamic Numeric Sender IDs are not supported by Telenor Montegro. Sender IDs will be overwritten with a Generic Alphanumeric Sender ID outside the Twilio platform to ensure delivery towards the specific network. | --- |
| Best practices | --- | --- | --- | --- |
...
- Two-Way Conversations: WhatsApp (You can use your existing number or get a new one from Bird) Supported**
- Alphanumeric Sender ID (Business names like "Bird" or "Netflix" that send one-way messages): Yes
- Luxembourg Phone Number: Yes
- Luxembourg Short Code: Yes
...
| Sender availability | - Alpha - Virtual Long Number - Short Code Generic senders are not allowed. |
| Sender provisioning | The average sender registration process time depends solely on network providers and it can take up to 5 days. |
| Two-way | Short Code |
| Two-way provisioning | The setup can take up to 1 month. |
| Country regulations | ASTW is set for promo traffic as 8:00 AM to 20:00 PM only on working days (during nights and weekends no promo allowed), as per country regulation. There is a regulatory DND database in Montenegro and if the end user registered their number, no promo message will be delivered to their number. |
