...
Source: https://www.twilio.com/en-us/guidelines/qa/sms
...
| --- | --- | --- |
| Locale name | --- | Qatar |
| ISO code | The International Organization for Standardization two character representation for the given locale. | QA |
| Region | --- | Middle East & Africa |
| Mobile country code | A three digit code associated with the given locale and used in conjunction with a Mobile Network Code to uniquely identify mobile networks. | 427 |
| Dialing code | The dialing prefix used to establish a call or send an SMS from one locale to the given locale. | +974 |
...
| Compliance considerations | Twilio strongly encourages customers to review proposed use cases with qualified legal counsel to make sure that they comply with all applicable laws. This table lists some general best practices. | Sender ID Compliance Sender ID Registration is required in Qatar. Starting on August 11, 2025, messages with unregistered Sender IDs will be blocked. To continue sending messages, you must use a registered Alphanumeric Sender ID. Content Compliance Requirements for SMS Delivery to Qatar Adult, political, governmental, religious, and cryptocurrency-related content is prohibited. Messages containing such content will be filtered out and accounts deny-listed. Additional Phone Numbers and Sender ID Guidelines Twilio strongly encourages customers to review proposed use cases with qualified legal counsel to make sure that they comply with all applicable laws. The following are some general best practices: Get opt-in consent from each end-user before sending any communication to them, particularly for marketing or other non-essential communications.Only communicate during an end-user’s daytime hours unless it is urgent.SMS campaigns should support HELP/STOP messages, and similar messages, in the end-user’s local language.Do not contact end-users on do-not-call or do-not-disturb registries. |
...
| Use case restrictions | --- | Adult, political, governmental, and religious content is prohibited. Messages containing such content will be filtered out and accounts deny-listed. | N/A |
| Best practices | --- | Local network operators tend to deactivate Sender IDs which were registered in the past but are not used for a considerable period of time. We suggest our clients making sure that they keep sending traffic periodically to the specific destination once they register their Alpha Sender IDs. If the Sender ID is deactivated due to inactivity then the customer needs to re-initiate the registration procedure. | N/A |
...
- Alphanumeric Sender ID (Business names like "Bird" or "Netflix" that send one-way messages): Yes (Alpha only, no numbers)
- Qatar Phone Number: No
- Qatar Short Code: No
...
- Sender registration is mandatory for all Qatar networks
- No Objection Certificate (NOC) is required for sender ID registration
...
| Service restrictions | Sender registration is a required. Documentation is not needed. Before you start sending any content towards Qatar for the first time, contact your dedicated account manager or [Support](mailto:support@infobip.com) so they can set up a specific route for you. |
| Service provisioning | Setup depends on sender provisioning time (depending on the network). |
| Sender availability | Alphanumeric, Alpha, Short Code |
| Sender provisioning | The average sender registration process time depends solely on network providers. Usually, it can take up to 3 days. |
| Two-way | Short Code |
| Two-way provisioning | 1-2 weeks but it can take longer depending on the MNO. |
| Country regulations | General regulation: A2P SMS traffic is divided on transactional and promotional. |
| Country restrictions | Gambling, betting, SPAM, loan traffic, Crypto, Forex and adult, political or religious content is likely to be blocked by the Qatari operators. No specific promo regulations. |
| Country recommendations | Highly regulated country. For more details, contact [Support](mailto:support@infobip.com) or your dedicated account manager. |